use super::jsondata::{LevelData, Object, SeedData};
...
pub fn get_walkable_exits(seed_data_json: &mut SeedData) {
    ...
    let new_walkable_exits = exits.clone();
    for exit in new_walkable_exits.iter() {
...
fn get_valid_exits<'a>(level_data: &'a mut LevelData, exits: &'a mut Vec<Object>) -> &'a mut Vec<Object> {
    // if there's a stretch of this many tiles at the edge of the map
    let connecting_tiles = 3..=50;
    // top exits
    if level_data.map[1].len() > 1 {
        let exit1 = level_data.map[1][1];
        if connecting_tiles.contains(&exit1) {
            let x: u32 = level_data.offset.x + level_data.map[1][0] as u32 + (level_data.map[1][1] as f32 / 2.) as u32;
            let y: u32 = level_data.offset.y;
            exits.push(Object::new_walkable_exit(x, y, level_data.id, exits));
